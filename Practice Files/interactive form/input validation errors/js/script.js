...
// 1c. Hide the `lastChild` of the `element` parameter's `parentElement`
...
// 2c. Display the `lastChild` of the `element` parameter's `parentElement`
...
const nameValidator = () => {
  ...
  const nameIsValid = /^[a-zA-Z]+ ?[a-zA-Z]*? ?[a-zA-Z]*?$/.test(user_name.value);

  // YOUR CODE GOES HERE!!!

  // 3a. Create an if/else statement.
  // If `nameIsValid` equals true, call the `validationPass` function and pass it the `name` variable from above as an argument
  // Else call the `validationFail` function and pass it the `name` variable from above as an argument

  if (nameIsValid === true) {
    validationPass(user_name);
  } else {
    validationFail(user_name);
  }
...
const languageValidator = () => {
  ...
  // Tests that the `languageTotal` variable provided above equals an integer greater than zero.
...
[
  ...document.querySelectorAll('#languages inputs'),
].forEach((cb) => {
