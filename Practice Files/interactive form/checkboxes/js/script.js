...
document.querySelector('.super-stats').addEventListener('change', (e) => {
  ...
  checkboxes.forEach((box) => {
    const checkboxType = box.getAttribute('data-type');

    if (clickedType === checkboxType && clicked !== box) {
      if (clicked.checked) {
        box.disabled = true;
      } else {
        box.disabled = false;
  ...
  [
    ...
    (cb) => (cb.disabled ? cb.parentElement.classList.add('disabled') : cb.parentElement.classList.remove('disabled'))
  );
});
